...
from sklearn.metrics import mean_squared_error, mean_absolute_error, r2_score, confusion_matrix, classification_report, roc_curve, auc, accuracy_score, f1_score, precision_score, recall_score
import os
...
def create_output_dirs():
    ...
    try:
        ...
        viz_dir = os.path.join("visualizations", "logistic_reg")
        os.makedirs(viz_dir, exist_ok=True)
        
        # Create model directory with timestamp
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        model_dir = os.path.join("models", f"logistic_reg_{timestamp}")
...
try:
    ...
    # Create binary target variable using quantiles
    ...
    # Define parameter grid for multi-class
    param_grid = {
        'C': [0.1, 1, 10],
        'penalty': ['l2'],
        'solver': ['lbfgs']  # Changed to lbfgs for multi-class
    }
    ...
    grid_search = GridSearchCV(
        ...
        cv=3,
        scoring='f1_weighted',  # Changed to weighted F1 for multi-class
        n_jobs=-1,
        verbose=1
    )
    ...
    y_test_prob = best_model.predict_proba(X_test_scaled)[:, 1]  # Probability of class 1
    ...
    feature_importance = pd.DataFrame({
        'Feature': features,
        'Importance': np.abs(best_model.coef_[0])  # Using first class coefficients
    })
