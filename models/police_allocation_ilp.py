...
class PoliceAllocationOptimizer:
    def __init__(self, crime_data_file):
        ...
        """
        # Use path in models directory
        self.crime_data = pd.read_csv('models/data/monthly_predicted_crime_density.csv')
        print("Available columns:", self.crime_data.columns.tolist())
        self.model = None
        self.solution = {}
        ...
        self.crime_data['Month'] = pd.to_datetime(self.crime_data['year_month'])
        self.crime_data['year'] = self.crime_data['Month'].dt.year
        self.crime_data['month'] = self.crime_data['Month'].dt.month
        
        # Rename predicted_density to crime_density
        self.crime_data.rename(columns={'predicted_density': 'crime_density'}, inplace=True)
        
        print("Columns after processing:", self.crime_data.columns.tolist())
    ...
    def generate_pdf_report(self, ward_id, month):
        ...
        pdf_path = os.path.join('models', 'reports', f'ward_{ward_id}_report_2025_{month:02d}.pdf')
        
        print(f"\nGenerating report at: {os.path.abspath(pdf_path)}")
...
if __name__ == "__main__":
    ...
    optimizer = PoliceAllocationOptimizer('models/data/monthly_predicted_crime_density.csv')
    
    # Run optimization
    if optimizer.optimize():
        # Generate report for specified ward and month
        print("\n" + "="*80)
        print(f"GENERATING REPORT FOR WARD {args.ward_id} - 2025-{args.month:02d}")
        print("="*80)
...
optimizer.generate_pdf_report('E05009305', month=1)  # High crime density ward
optimizer.generate_pdf_report('E05009308', month=1)  # Very high crime density ward
optimizer.generate_pdf_report('E05009317', month=1)  # Low crime density ward
optimizer.generate_pdf_report('E05009395', month=1)  # Medium-high crime density ward
